...
// 1. INTENT SEARCH BAR COMPONENT (REDESIGNED - PREMIUM v2)
    ...
    const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (lowerVal.includes('costru') || lowerVal.includes('casa') || lowerVal.includes('villa')) {
            setIntentResult({
                ...
                icon: Hammer,
                desc: 'Analisi lotti edificabili per progetti residenziali.',
                action: 'analysis'
            });
            ...
            setIntentResult({
                ...
                icon: DollarSign,
                desc: 'Valutazione immediata e matching con investitori.',
                action: 'sell'
            });
            ...
            setIntentResult({
                ...
                icon: TrendingUp,
                desc: 'Calcolo ROI per Fotovoltaico, Glamping o Logistica.',
                action: 'income'
            });
            ...
            setIntentResult({
                ...
                icon: Sprout,
                desc: 'Analisi morfologica per colture ad alto valore.',
                action: 'analysis'
            });
        } else {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto relative z-50">
            <div className={`relative group transition-all duration-500 ${isFocused ? 'scale-105' : ''}`}>
                {/* Glow Effect */}
                <div className={`absolute -inset-1 bg-gradient-to-r from-primary-500 via-purple-500 to-primary-500 rounded-2xl blur-xl opacity-20 group-hover:opacity-40 transition duration-1000 ${isFocused ? 'opacity-60 blur-2xl' : ''}`}></div>

                {/* Input Container */}
                <div className="relative bg-[#09090b]/90 backdrop-blur-2xl rounded-2xl border border-white/10 flex flex-col shadow-2xl overflow-hidden transition-all">

                    {/* Input Area */}
                    <div className="flex items-center p-2">
                        <div className="p-4">
                            <Sparkles className={`w-6 h-6 text-primary-500 ${text.length > 0 ? 'animate-spin-slow' : 'animate-pulse'}`} />
                        </div>
                        <input
                            type="text"
                            value={text}
                            ...
                            className="w-full bg-transparent border-none text-white text-xl md:text-2xl font-medium placeholder:text-gray-600 focus:ring-0 outline-none h-16"
                            placeholder="Descrivi il tuo progetto (es. 'Voglio costruire una villa in Toscana')..."
                        />
                        {text.length > 0 && (
                            <button onClick={() => setText('')} className="p-4 text-gray-500 hover:text-white transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        )}
                    </div>

                    {/* Suggestions / Results Area (Expands) */}
                    {(isFocused || text.length > 0) && (
                        <div className="border-t border-white/5 bg-black/20 p-2 animate-slide-down">

                            {text.length > 0 && intentResult ? (
                                <div
                                    onClick={() => executeAction(intentResult.action)}
                                    className="flex items-center gap-4 p-4 rounded-xl bg-primary-500/10 border border-primary-500/20 cursor-pointer hover:bg-primary-500/20 transition-all group mx-2 mb-2"
                                >
                                    ...
                                    <div className="bg-primary-500 text-black px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 group-hover:scale-105 transition-transform">
                                        VAI <ArrowRight className="w-4 h-4" />
                                    </div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-2">
                                    <div className="text-[10px] text-gray-500 uppercase tracking-widest font-bold col-span-full mb-1 pl-2">Suggerimenti Rapidi</div>
                                    {[
                                        { text: "Costruire Casa", sub: "Analisi edificabilità", icon: Home, action: 'analysis' },
                                        { text: "Vendere Terreno", sub: "Valutazione gratuita", icon: DollarSign, action: 'sell' },
                                        { text: "Investire nel Solare", sub: "Calcolo rendita", icon: Sun, action: 'income' },
                                        { text: "Avviare Glamping", sub: "Business Plan", icon: Tent, action: 'income' }
                                    ].map((chip, i) => (
                                        <button
                                            key={i}
                                            onMouseDown={() => executeAction(chip.action)}
                                        ...
                                        >
                                            <chip.icon className="w-5 h-5 text-gray-500 group-hover:text-white transition-colors" />
                                            <div>
                                                <div className="text-sm text-gray-300 group-hover:text-white font-medium transition-colors">{chip.text}</div>
                                                <div className="text-xs text-gray-600 group-hover:text-gray-400">{chip.sub}</div>
                                            </div>
                                        </button>
                                    ))}
                                </div>
...
const AIQuerySection = ({ onNavigate }: { onNavigate: (mode: string) => void }) => {
    return (
        <div className="py-24 bg-black relative overflow-hidden border-y border-white/5">
            {/* Background Gradients */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full max-w-7xl opacity-20 pointer-events-none">
                <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary-500/20 rounded-full blur-[100px] mix-blend-screen"></div>
                <div className="absolute top-1/3 right-1/4 w-96 h-96 bg-purple-500/20 rounded-full blur-[100px] mix-blend-screen"></div>
            </div>

            <div className="max-w-7xl mx-auto px-4 relative z-10 text-center">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-8 backdrop-blur-md">
                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                    <span className="text-xs font-mono text-gray-400 uppercase tracking-widest">AI Neural Engine v2.0</span>
                </div>
                ...
                <p className="text-gray-400 text-lg mb-12 max-w-2xl mx-auto leading-relaxed">
                    Descrivi la tua idea. La nostra AI analizzerà migliaia di ettari per trovare l'opportunità perfetta per te, calcolando costi e ricavi in tempo reale.
                </p>

                <IntentSearchBar onNavigate={onNavigate} />
            </div>
        </div>
    );
};
        ...
        <div className="w-full max-w-xl mx-auto lg:ml-auto relative z-20 animate-float">
            ...
            <div className="flex space-x-1 mb-2 p-1 bg-black/40 backdrop-blur-md rounded-t-xl border border-white/5 w-fit">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 uppercase tracking-wide ${activeTab === 'buy' ? 'bg-white text-black shadow-lg' : 'text-gray-400 hover:text-white'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 uppercase tracking-wide ${activeTab === 'sell' ? 'bg-primary-500 text-black shadow-lg shadow-primary-500/20' : 'text-gray-400 hover:text-white'
                        }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 uppercase tracking-wide ${activeTab === 'income' ? 'bg-accent-500 text-white shadow-lg shadow-accent-500/20' : 'text-gray-400 hover:text-white'
                        }`}
                >
